...
COLORS = {
    ...
    "accent_glow": "#0d4d30",  # Glow effect (green mixed with dark bg)
    "accent_hover": "#00ffaa", # Brighter on hover
    "accent_dim": "#00aa55",   # Dimmed for inactive
    "accent_border": "#1a5540", # Glowing borders (30% opacity green on dark)
...
class ModernSettingsWindow:
    ...
    def _load_custom_fonts(self):
        """Try to load custom fonts for better typography"""
        # CustomTkinter doesn't support custom font loading easily
        # We'll use system fonts with fallbacks
        # Outfit (if installed) or fallback to system fonts
        self.font_heading = "Outfit"  # Will fallback to default if not available
        self.font_mono = "JetBrains Mono"
        self.font_body = "Segoe UI"

    def _build_layout(self):
        """Build the main layout with premium sidebar and content area"""
        # Main container with padding
        container = ctk.CTkFrame(self.root, fg_color="transparent")
        container.pack(fill="both", expand=True, padx=28, pady=28)

        # Left sidebar with subtle gradient effect (simulated with layers)
        sidebar = ctk.CTkFrame(container, width=260, fg_color=COLORS["panel"], corner_radius=20)
        sidebar.pack(side="left", fill="y", padx=(0, 24))
        sidebar.pack_propagate(False)

        # Header with logo and branding
        header_frame = ctk.CTkFrame(sidebar, fg_color="transparent")
        header_frame.pack(fill="x", padx=24, pady=(28, 40))
        ...
        try:
            ...
            if logo_path.exists() and PIL_AVAILABLE:
                # Load and resize image (larger for more impact)
                self._logo_pil_image = Image.open(logo_path)
                self._logo_pil_image = self._logo_pil_image.resize((64, 64), Image.Resampling.LANCZOS)

                # Use CTkImage for proper HighDPI support
                self._logo_image = ctk.CTkImage(light_image=self._logo_pil_image, dark_image=self._logo_pil_image, size=(64, 64))
        ...
        # Text branding - centered with better typography
        ...
        subtitle_app.pack(anchor="center")

        # Subtle divider with glow effect
        divider_container = ctk.CTkFrame(header_frame, fg_color="transparent", height=20)
        divider_container.pack(fill="x", pady=(16, 0))
        divider = ctk.CTkFrame(divider_container, height=1, fg_color=COLORS["accent_border"])
        divider.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.6)

        # Navigation buttons with real icons
        nav_items = [
            ("General", "⚙", "GENERAL"),
            ("Audio", "◉", "AUDIO"),
            ("Overlay", "◐", "OVERLAY"),
        ...
        nav_container.pack(fill="x", padx=16, pady=(8, 16))
        ...
        # Content area with better scrollbar
    ...
    def _build_card(self, parent, title: str) -> ctk.CTkFrame:
        """Create a premium card container with title and accent border"""
        # Single card with border (simpler, no rendering issues)
        card = ctk.CTkFrame(
            parent,
        ...
        title_container.pack(fill="x", padx=32, pady=(24, 20))

        # Accent indicator (vertical bar)
        ...
        title_label = ctk.CTkLabel(
            title_container,
            text=title.upper(),
            font=(self.font_heading, 20, "bold"),
    ...
    def _build_general(self):
        ...
        card = self._build_card(frame, "General Settings")
        content = ctk.CTkFrame(card, fg_color="transparent")
        content.pack(fill="x", padx=32, pady=(0, 24))

        # Hotkey with capture button - improved layout
        row = ctk.CTkFrame(content, fg_color="transparent")
        row.pack(fill="x", pady=(0, 16))
        label_col = ctk.CTkFrame(row, fg_color="transparent", width=180)
        ...
        # Activation mode - improved with better spacing
        row = ctk.CTkFrame(content, fg_color="transparent")
        row.pack(fill="x", pady=(0, 16))
        label_col = ctk.CTkFrame(row, fg_color="transparent", width=180)
        label_col.pack(side="left", fill="y")
        ...
        rb1 = ctk.CTkRadioButton(
            mode_frame,
            text="Push-to-Talk",
            variable=self.vars["activation_mode"],
            value="ptt",
        ...
        cb = ctk.CTkCheckBox(
            ...
            variable=self.vars["start_on_boot"],
            font=(self.font_body, 14),
            command=self._auto_save,
            fg_color=COLORS["accent"],
            hover_color=COLORS["accent_hover"],
        ...
        self.vars["paste_method"] = ctk.StringVar(value="ctrl_v")
        ...
        self.vars["clipboard_policy"] = ctk.StringVar(value="copy_to_clipboard")
        combo = ctk.CTkComboBox(
            row,
            variable=self.vars["clipboard_policy"],
            values=["dont_modify", "copy_to_clipboard"],
            width=260,
            height=48,
            corner_radius=12,
    ...
    def _build_audio(self):
        ...
        card = self._build_card(frame, "Audio Settings")
        content = ctk.CTkFrame(card, fg_color="transparent")
        content.pack(fill="x", padx=32, pady=(0, 24))
    ...
    def _build_overlay(self):
        ...
        card = self._build_card(frame, "Overlay Settings")
        content = ctk.CTkFrame(card, fg_color="transparent")
        content.pack(fill="x", padx=32, pady=(0, 24))
        ...
        ctk.CTkCheckBox(
            row,
            text="Show overlay",
            variable=self.vars["overlay_enabled"],
            font=(self.font_body, 14),
            command=self._auto_save,
            fg_color=COLORS["accent"],
            hover_color=COLORS["accent_hover"],
        ...
        ctk.CTkComboBox(
            row,
            variable=self.vars["overlay_position"],
            values=["bottom", "top"],
            width=260,
            height=48,
            corner_radius=12,
        ...
        ctk.CTkSlider(
            ...
            from_=0.5,
            to=1.0,
            variable=self.vars["overlay_opacity"],
            width=320,
            height=20,
    ...
    def _build_post(self):
        ...
        card = self._build_card(frame, "AI Post-Processing")
        content = ctk.CTkFrame(card, fg_color="transparent")
        content.pack(fill="x", padx=32, pady=(0, 24))
        ...
        entry = ctk.CTkEntry(
            row,
            textvariable=self.vars["pp_api_key"],
            width=420,
            height=48,
            corner_radius=12,
            font=(self.font_mono, 13),
        ...
        search_entry = ctk.CTkEntry(
            filter_row,
            textvariable=self.vars["model_search"],
            width=240,
            height=40,
            corner_radius=12,
        ...
        # Model list
        self.model_list_frame = ctk.CTkScrollableFrame(
            content,
            fg_color=COLORS["panel"],
            height=200,
            border_width=0,
        ...
        add_entry = ctk.CTkEntry(
            add_row,
            textvariable=self.vars["add_model"],
            width=420,
            height=44,
            corner_radius=12,
            font=(self.font_mono, 12),
            ...
            border_color=COLORS["border"],
            fg_color=COLORS["panel"],
            placeholder_text="vendor/model-name"
        )
        add_entry.pack(side="left", padx=(0, 12))
    ...
    def _build_about(self):
        ...
        ctk.CTkLabel(
            content,
            text="Whisper Cheap",
            font=(self.font_heading, 28, "bold"),
            text_color=COLORS["accent"]
        ...
        ctk.CTkLabel(
            content,
            text="Local voice transcription powered by Parakeet V3",
            font=(self.font_body, 14),
            text_color=COLORS["text_dim"]
    ...
    def _show_section(self, name: str):
        """Show the selected section and update nav button states"""
        ...
        # Update nav button states with glow effect for active item
        for sec, btn in self.nav_buttons.items():
            if sec == name:
                # Active state: accent background with black text
                btn.configure(
                    fg_color=COLORS["accent"],
                    text_color="#000000",
                    hover_color=COLORS["accent_hover"],
                ...
                # Inactive state: transparent with dim text
                btn.configure(
                    fg_color="transparent",
                    text_color=COLORS["text_dim"],
                    hover_color=COLORS["panel_hover"],
